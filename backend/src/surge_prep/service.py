...
# How far back to look when resolving the live session for a device. `list_sessions`
# is paged newest-first, and a device's active session is always among the latest few.
RECENT_SESSION_SCAN_LIMIT = 200
...
class TrainingService:
    ...
    async def _retire_active_sessions_for_device(self, device_id: str) -> None:
        ...
        session per device.
        """
        for other in await self.store.list_sessions(RECENT_SESSION_SCAN_LIMIT):
    ...
    async def get_active_session(self) -> Session:
        # `list_sessions` is newest-first, so the first active one is the newest.
        sessions = await self.store.list_sessions(RECENT_SESSION_SCAN_LIMIT)
