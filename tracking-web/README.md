...
- Relative Z change **only after** the stable starting position is captured. The estimate is `100 × (starting marker size ÷ current marker size − 1)` percent of the starting distance.
- Detection loss, shown visibly; stale positions, marker size, relative Z, and speed are cleared immediately. The path resumes as a separate segment after reacquisition; it does not fill a gap with interpolated measurements.

Processing is limited to 480 pixels wide and at most about 30 new video frames per second to keep the browser responsive. Fast motion can still blur a marker or exceed the camera's own frame rate; a lost marker is shown as lost instead of extrapolating a fictitious position. The relative Z estimate assumes the marker keeps the same orientation toward the camera; tilt, oblique views, and lens distortion change the apparent size and therefore bias the result. It is **not** calibrated all-side 3D pose, and X/Y remain image pixels. This experiment performs no camera-intrinsic calibration, tip-offset correction, force/contact sensing, or anatomy registration. It does not record or transmit video frames. The detector runs locally in the browser.
